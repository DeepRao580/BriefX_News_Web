  ...
  useEffect(() => {
    const fetchRecentNews = async () => {
      try {
        setLoading(true);
        if(!navigator.onLine){
          const offlineNews = await getOfflineNews("health");
        ...
        const response = await fetch(
          `https://api.currentsapi.services/v1/search?keywords=health&language=${lang}&country=IN&page_size=20`,
          {
            headers: {
        ...
        await saveOfflineNews("health",data.news)
      ...
      <div className="mx-auto w-full max-w-[1800px] px-3 sm:px-4 md:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-6 lg:grid-cols-2 lg:gap-8 xl:grid-cols-3 xl:gap-10">
          ...
          {allRecentNews.map((singleNews, index) => (
            <NewsCard key={index} singleNews={singleNews} category="health" />
          ))}
